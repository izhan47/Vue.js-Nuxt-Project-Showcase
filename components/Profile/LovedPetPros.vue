...
<div >
  ...
  <v-row v-if="petLoved.length">
    <v-col cols="12" md="6" sm="12"  v-for="item in petLoved">
  <div class="card-section mb-3">
    <div v-if="item.pet_pro.cover_image" class="rating-section">
      <nuxt-link :to="`/pet-category/${item.pet_pro.slug}`" >
        <div  class="bg-img-height" :style="`background-image: url(${item.pet_pro.cover_image.image_full_path})`">
        </div>
      ...
      <div class="delete-action" @click="removeLove(item.pet_pro.slug)" >
        <img class="img-fluid img-height" src="/images/minus.svg" alt="">
      </div>
    </div>
    <div v-else>
      <nuxt-link :to="`/pet-category/${item.pet_pro.slug}`" >
    ...
    <div class="card-category" @click="$router.push(`/pet-category/${item.pet_pro.slug}`)">
      ...
      <v-btn v-if="item.pet_pro.deals_count !== 0 "   class="cat-btn"  outlined rounded>
...
export default {
  ...
  async  lovedPetPro(){
    ...
    await this.$store.dispatch('getLovedPet')
      .then(response => {
